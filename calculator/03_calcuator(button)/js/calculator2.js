...
function edit(wkInput) {
  ...
  // let newValue = elementResult.innerHTML.toString()+ wkInput;
  // if (newValue.length > 10) return;
  // elementResult.innerHTML = Number(newValue);
  }
  // １つ前の入力が、演算子
  else {
...
function update(calcType) {
  ...
  if (wkBefore === "0") {
    elementcalcLog.innerHTML = elementcalcLog.innerHTML
      + Number(elementResult.innerHTML)
...
function calculator() {
  switch (wkCalc) {
    case "+":
      wkTotal = +wkTotal + +elementResult.innerHTML
      break;
    case "-":
      wkTotal = +wkTotal - +elementResult.innerHTML
      break;
