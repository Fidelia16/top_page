...
squaresArray.forEach(square => {
    square.addEventListener("click", () => {
        ...
        if (gameOverFlg === "0") {
            const squaresBox = document.getElementById("squares-box");
...
function isWinner(symbol) {
    const result = lineArray.some(function (line) {
        const subResult = line.every(function (square) {
            if (symbol === "penguins") return square.classList.contains("js-pen-checked"); 
            if (symbol === "bear") return square.classList.contains("js-bear-checked"); 
        });
